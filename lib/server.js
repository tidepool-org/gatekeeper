...
module.exports = function (userApiClient, dataBroker) {
  function createServer(serverConfig) {
    ...
    retVal.get('/access/groups/:userId', checkToken, function(req, res, next){
      var tokendata = req._tokendata;
      if (tokendata.isserver || req.context.userId === tokendata.userid) {
        return dataBroker.groupsForUser(req.context.userId, resultsCb(req, res, next));
      } else {
    ...
    retVal.get('/access/:groupId', checkToken, function(req, res, next){
      var tokendata = req._tokendata;
      if (!tokendata.isserver && req.context.groupId !== tokendata.userid) {
        res.send(401, 'These are not the droids you are looking for.');
        return next();
      }

      dataBroker.usersInGroup(req.context.groupId, resultsCb(req, res, next));
    });

    retVal.get('/access/:groupId/:userId', checkToken, function(req, res, next){
      var tokendata = req._tokendata;
      if (tokendata.isserver || req.context.groupId === tokendata.userid || req.context.userId === tokendata.userid) {
        dataBroker.userInGroup(req.context.userId, req.context.groupId, resultsCb(req, res, next));
      } else {
        dataBroker.usersInGroup(req.context.groupId, function(err, results){
          if (err != null) {
            log.info(err, 'Error on checking usersInGroup[%s]', req.context.groupId);
            res.send(500);
            return next();
          }
          ...
          var retVal = results[req.context.userId];
          if (retVal == null) {
            res.send(404);
          } else {
            res.send(200, retVal);
    ...
    retVal.post('/access/:groupId/:userId', checkToken, function(req, res, next){
      var tokendata = req._tokendata;

      function doIt() {
        var cb = function (err, results) {
          if (err != null) {
            log.info(err, 'Error on url[%s]', req.url);
            res.send(500);
          } else {
        ...
        var perms = req.body;
        if (Buffer.isBuffer(perms) && perms.length === 0) {
          perms = {};
        }

        dataBroker.setPermissions(req.context.userId, req.context.groupId, perms, function(err){
          if (err != null) {
            return cb(err);
          } else {
            dataBroker.userInGroup(req.context.userId, req.context.groupId, cb);
          }
        });
      }
      ...
      if (tokendata.isserver || req.context.groupId === tokendata.userid || req.context.userId === tokendata.userid) {
        ...
        dataBroker.userInGroup(tokendata.userid, req.context.groupId, function(err, results) {
          if (err != null) {
            log.info(err, 'Error on checking userInGroup[%s]', req.context.groupId);
            res.send(500);
            return next();
          }

          if (results == null || results.admin == null) {
