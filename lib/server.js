...
module.exports = function (userApiClient, dataBroker) {
  function createServer(serverConfig) {
    ...
    retVal.post('/access/:groupId/:userId', checkToken, function(req, res, next){
      ...
      function doIt(authenticatedPerms) {
        ...
        var requestedPerms = req.body;
        if (Buffer.isBuffer(requestedPerms) && requestedPerms.length === 0) {
          requestedPerms = {};
        }

        if (authenticatedPerms == null || (authenticatedPerms.root == null && authenticatedPerms.admin == null && !_.isEmpty(requestedPerms))) {
          res.send(401, 'Nope nope nope');
          return next();
        }

        dataBroker.setPermissions(req.params.userId, req.params.groupId, requestedPerms, function(err){
          if (err != null) {
            return cb(err);
          } else {
