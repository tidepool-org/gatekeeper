...
module.exports = function (userApiClient, dataBroker) {
  function createServer(serverConfig) {
    ...
    retVal.get('/access/groups/:userid', checkToken, permissions.requireCustodian, function(req, res, next) {
      dataBroker.groupsForUser(req.params.userid, resultsCb(req, res, next));
    });

    retVal.get('/access/:userid', checkToken, permissions.requireCustodian, function(req, res, next) {
      dataBroker.usersInGroup(req.params.userid, resultsCb(req, res, next));
    });

    retVal.get('/access/:userid/:granteeid', checkToken, requireReadPermissions, function(req, res, next) {
      dataBroker.userInGroup(req.params.granteeid, req.params.userid, resultsCb(req, res, next));
    });

    retVal.post('/access/:userid/:granteeid', checkToken, normalizePermissionsBody, requireWritePermissions, function(req, res, next) {
