...
/**
 * This class exists in order to wrap the "good" broker and add a compatibility layer with the old way of
 * verifying permissions.  Once everything has updated, this file should be eliminated along with the
 * package.json depenencies on tidepool-armada-client and tidepool-seagull-client
 *
 * @param userApiClient
 * @param seagullClient
 * @param armadaClient
 * @param broker
 * @returns {{userInGroup: userInGroup, usersInGroup: usersInGroup, groupsForUser: groupsForUser, setPermissions: (function(this:*))}}
 */
  ...
  function getMembersOfGroupNamed(userId, group, cb) {
    userApiClient.withServerToken(function (tokenErr, token) {
      ...
      seagullClient.getGroups(userId, token, function (err, groups) {
        ...
        var viewersGroupId = groups == null ? null : groups[group];
        ...
        function(err, results) {
          ...
          async.map(
            ...
            function(err, updates) {
              if (updates.every(function(e){ return e === 'not updated'; })) {
                return cb(null, results.gatekeeper);
              }
        ...
        function(err, results) {
          ...
          async.map(
            ...
            function(groupId, asyncCb) {
              if (results.gatekeeper[groupId] == null) {
                broker.setPermissions(userId, groupId, {view: {}}, asyncCb);
              } else {
                asyncCb(null, 'not updated');
              }
            ...
            function(err, updates) {
              ...
              broker.groupsForUser(userId, cb);
            }
          );
        }
      );
